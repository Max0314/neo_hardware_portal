...
| 1 | **停机的具体日期与时段** | 你 |
| 2 | 旧服务作为只读回滚点保留多久 | 你 |
| 3 | BI 切换时机：何时改 `HARDWARE_BI_BASE_URL` | 你 |
| 4 | 报告审核迁移细节：给它加 base path 支持，还是继续背 `sub_filter` 这个技术债 | 你 |
| 5 | OSS 何时申请 | 陈龙 |
| 6 | 宜搭白名单是否现在收尾（72 张表，与迁移无关但可闭环） | 你 |
| 7 | `线材物料优选库` 源表物料代码列没填 | 业务侧 |
| 8 | `CPU&WIFI芯片` 源表 0 条实例，是否本该有数据 | 业务侧 |
| 9 | 旧服务器 `MYSQL_PASSWORD` 轮换（部署脚本曾明文打印到日志） | 你 |
