...
**已决定（2026-08-07）：此反代即为长期方案**，两台服务器持续并用；仅当 bi_center
自身迁往阿里云时才移除。8/12 只清理旧门户的容器/卷/备份，**Nginx 里这段转发块不动**
（配置内已标注"长期方案，勿删"）。已知依赖：新服务器公网 IP 变更时须同步改该
proxy_pass，否则 BI 取数会静默中断。
