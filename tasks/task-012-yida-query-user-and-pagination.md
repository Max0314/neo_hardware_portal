...
- 已配置：`533918221524183112`（张志伟，工号 20461992，硬件研发部-网关电路设计组）。已实测该
  账号对全部 72 张物料表单有数据权限。写入前已备份 `.env.bak-20260804-112056`，权限保持 600。
- **取值决策**：经确认，长期使用该账号，不另行申请专用服务账号。需要知悉的后果是：该账号一旦
  调岗、离职或数据权限变更，同步会再次全线取不到数据。task-011 的空结果保护会阻止覆盖，且本
  任务已让「0 实例」直接指向查询人权限，因此故障会立即暴露而不再静默损坏数据。
...
- 分页修复后单次同步的实例量从 2,360 升至 2,872，物料行数会明显上升。task-011 的减量保护只
  拦下降不拦上升，不会误阻断。
- 未完成项：
  1. `线材物料优选库` 的物料代码列需业务侧在宜搭源表补录。
  2. `CPU&WIFI芯片` 源表 0 条实例，需业务侧确认是否应有数据。
  3. 恢复同步后应复核 5 张受截断影响表单的物料行数是否相应上升。
